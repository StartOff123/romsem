...
const Slider = () => {
    ...
    const [sliderWidth, setSliderWidth] = React.useState<number | undefined>(ref.current?.scrollWidth);
    const [sliderWidthItem, setSliderWidthItem] = React.useState<number | undefined>(ref.current?.scrollWidth);
    ...
    return (
        <div className='flex flex-col gap-5'>
            <div ref={ref} className='relative overflow-hidden flex h-[600px] rounded-md'>
                {(sliderWidth && sliderWidthItem) && (
                    <div
                        style={{ width: sliderWidth, transform: `translateX(-${sliderWidthItem * (currentSlide - 1)}px)` }}
                    ...
                    >
                        {sliderItems.map((item, i) => (
                            <div
                                ...
                                style={{ background: `url(${item.image}) no-repeat`, backgroundSize: 'cover', backgroundPosition: 'center', width: sliderWidthItem }}
                            >
            ...
            <div className='flex items-center gap-2'>
                <button onClick={() => handleClickArrow(-1)} className='text-indigo-500 transition-colors hover:text-indigo-300'>
                    <FaArrowLeftLong size={14} />
                </button>
                <span className='text-sm'>{currentSlide}/{sliderItems.length}</span>
                <button onClick={() => handleClickArrow(1)} className='text-indigo-500 transition-colors hover:text-indigo-300'>
