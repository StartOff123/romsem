...
import { addCart, removeCart } from '@/lib/redux/slices/cart-slice';
...
const Card = ({ item, isAdded, actionsLoading }: CardProps) => {
    ...
    // @ts-ignore
    const details = JSON.parse(item.details) as { volume?: string, size?: string, pieces?: string };
    ...
    return (
        <div ref={ref} className='relative group max-w-[280px] w-full rounded-t-md transition-shadow hover:shadow-[0px_0px_20px_0px_rgba(0,0,0,0.2)] cursor-pointer'>
            ...
            <div className='flex flex-col py-2 px-4 gap-2'>
                <span className='text-lg font-bold text-indigo-500'>{numberWithSpaces(Number(item.price))} ₽</span>
                <h1 className='text-base font-semibold text-ellipsis text-nowrap overflow-hidden'>{item.title}</h1>
            ...
            <div className='absolute flex gap-2 bg-white w-full rounded-b-md px-4 py-2 transition-opacity opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto shadow-[0px_0px_20px_0px_rgba(0,0,0,0.2)]'>
                ...
                {
                    isAdded && (
                        <button onClick={handleRemoveClick} className='px-3 bg-red-500 rounded-md text-white transition-colors hover:bg-red-600'>
                            <LuTrash2 size={18} />
                        </button>
                    )
                }
