...
import HeaderSearch from '@/modules/header/header-search';
...
const Header = () => {
	...
	React.useEffect(() => {
		if (window.localStorage.getItem('cart')) {
			dispatch(
				fetchCartItems(JSON.parse(window.localStorage.getItem('cart')!))
			);
		}
	}, []);
			...
			<Container>
				<div className="grid gap-3">
					<div className="relative flex justify-between items-center pt-2 before:absolute before:-bottom-1.5 before:w-full before:h-[1px] before:bg-zinc-300">
						...
						<Link
							href="/cart"
							className="flex items-center text-base gap-3 transition-colors hover:text-indigo-500 cursor-pointer"
						>
							<div className="relative">
								<HiOutlineShoppingBag size={25} />
								<span className="absolute bg-indigo-500 text-white top-0 right-0 px-1 min-w-[12px] text-[8px] leading-3 rounded-full">
									{cart?.length}
								</span>
							</div>
							Корзина
						...
						<ul className="flex gap-8 text-zinc-600">
							{categoriesList.map((item) => (
								<li key={item.key}>
									<Link
										href={item.href}
										className="flex items-center gap-3 transition-colors hover:text-indigo-500 cursor-pointer"
									>
										{item.icon}
										{item.text}
									</Link>
