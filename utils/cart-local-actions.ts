export const addCartLocal = (id: string) => {
    if (window.localStorage.getItem('cart')) {
        const currentCartItemId: string[] = JSON.parse(window.localStorage.getItem('cart')!) as string[];
...
export const removeCartLocal = (id: string) => {
    ...
    window.localStorage.setItem('cart', JSON.stringify(currentCartItemId.filter(item => item !== id)));
}